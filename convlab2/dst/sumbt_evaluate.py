...
import jieba
import json
from convlab2.dst.sumbt.multiwoz.bert_sumbt import SUMBTTracker
import os
...
if __name__ == '__main__':
    ...
    ## init phase
    print(sys.argv[1])
    config_data = json.load(open(sys.argv[1],'r'))
    data_key = sys.argv[2]
    model = SUMBTTracker(arg_path = sys.argv[1])
    ...
    if dataset_name.startswith('MultiWOZ'):
        ...
        #dataloader = AgentDSTDataloader(dataset_dataloader=MultiWOZDataloader(config_data['lang']=='zh'))
        dataloader = AgentDSTDataloader(dataset_dataloader=MultiWOZDataloader(True))
        #dataloader.load_data(data_key=data_key)[data_key]
        data = dataloader.load_data(data_key=data_key, data_dir=os.path.join('/fs/startiger0/nmoghe/code/ConvLab-2',config_data['data_dir']))[data_key]
        ...
        slot_list = multiwoz_zh_slot_list if config_data['lang']=='zh' else multiwoz_en_slot_list
