...
if __name__ == '__main__':
    ...
    if dataset_name.startswith('MultiWOZ'):
        if dataset_name.endswith('zh'):
            if model_name == 'sumbt':
                from convlab2.dst.sumbt.multiwoz_zh.sum2bt import SUMBTTracker
                model = SUMBTTracker()
            else:
        ...
        dataloader = AgentDSTDataloader(dataset_dataloader=MultiWOZDataloader(not(dataset_name.endswith('zh'))))
        data = dataloader.load_data(data_dir='/fs/startiger0/nmoghe/code/ConvLab-2/data/zh_data_en_states', data_key=data_key)[data_key]
        ...
        if len(curr_sess) > 0:
            all_predictions[session_count] = copy.deepcopy(curr_sess)
        json.dump(all_predictions, open(sys.argv[4],'w'))

        slot_list = multiwoz_slot_list if dataset_name.endswith('zh') else multiwoz_zh_slot_list
